...
public class ModesButton extends CompositeGUIComponent{
	...
	public static final Vector2 PLATFORM_SCALE_MAX = androidInstance ? ColorSwitch.ANDROID_SCALE : DESKTOP_SCALE_MAX;
	...
	public ModesButton(Texture buttonTexture, Texture arrowTexture, Screen owner) {
		...
		this.modes.setBounds((ColorSwitch.getInstance().getConfig().width - this.modes.getWidth()) / 2, ColorSwitch.getInstance().getConfig().height / 2 - this.modes.getHeight() * PLATFORM_SCALE_MAX.x * (androidInstance ? 3.5f : 4), this.modes.getWidth(), this.modes.getHeight());
		...
		this.arrows[1].setBounds(androidInstance ? this.modes.getX() - this.arrows[1].getHeight() : 0, this.modes.getY() + 20, this.arrows[1].getWidth(), this.arrows[1].getHeight());
		this.arrows[0].setBounds(this.modes.getWidth() * PLATFORM_SCALE_MAX.x + this.arrows[0].getWidth() * PLATFORM_ARROW_SCALE.x + (androidInstance ? 23 : 0), this.arrows[1].getY(), this.arrows[0].getWidth(), this.arrows[0].getHeight());
	}
	...
	public void draw(float deltaTime) {
		...
		if(this.isModesAtMaxScale && this.modes.getScaleX() >= PLATFORM_SCALE_MIN.x && this.modes.getScaleY() >= PLATFORM_SCALE_MIN.y) {
			this.modes.setScale(this.modes.getScaleX() - actualScaleSpeed, this.modes.getScaleY() - actualScaleSpeed);
			this.arrows[0].setScale(this.arrows[0].getScaleX() + actualScaleSpeed * (androidInstance ? 2.5f : 4), this.arrows[0].getScaleY() + actualScaleSpeed * (androidInstance ? 2.5f : 4));
			this.arrows[1].setScale(this.arrows[0].getScaleX(), this.arrows[0].getScaleY());
		}else {
			this.isModesAtMaxScale = false;
		}

		if(!this.isModesAtMaxScale && this.modes.getScaleX() <= PLATFORM_SCALE_MAX.x && this.modes.getScaleY() <= PLATFORM_SCALE_MAX.y) {
			this.modes.setScale(this.modes.getScaleX() + actualScaleSpeed, this.modes.getScaleY() + actualScaleSpeed);
			this.arrows[0].setScale(this.arrows[0].getScaleX() - actualScaleSpeed * (androidInstance ? 2.5f : 4), this.arrows[0].getScaleY() - actualScaleSpeed * (androidInstance ? 2.5f : 4));
			this.arrows[1].setScale(this.arrows[0].getScaleX(), this.arrows[0].getScaleY());
		}
